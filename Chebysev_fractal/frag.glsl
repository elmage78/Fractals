...
vec2 ddpoly(vec2 z, vec2[MAX_DEGREE + 1] coefs){
    vec2 result = vec2(0.0);
    for(int n = 2; n < int(n_roots) + 1; n++){
        result += n * (n-1) * complex_mult(coefs[n], complex_pow(z, n - 2));
    }
    return result;
}
...
vec2 seek_root(vec2 z, vec2[MAX_DEGREE + 1] coefs, int max_steps, out float n_iters){
    ...
    for(int i = 0; i < max_steps; i++){
        last_len = curr_len;
        n_iters = float(i);
        vec2 step = complex_div(poly(z,coefs),dpoly(z,coefs))+complex_mult(complex_pow(complex_div(poly(z,coefs),dpoly(z,coefs)),2),complex_div(ddpoly(z,coefs),2*dpoly(z,coefs)));
